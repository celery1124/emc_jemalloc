...
/* Data. */
int driver_fd;
bool driver_fd_open = false;
//MEM_CHUNK_TABLE mapping_table[512];
...
malloc_init(void)
{
	if (driver_fd_open == false)
	{
		driver_fd_open = true;
		//memset(mapping_table,0,512*sizeof(MEM_CHUNK_TABLE));
		if ((driver_fd = open("/dev/mem_driver", O_RDWR)) < 0) {
